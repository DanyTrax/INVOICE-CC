    ...
    <div class="space-y-6">
        ...
        <div class="border-b border-gray-200">
            <nav class="flex -mb-px" aria-label="Tabs">
                @php
                    $activeSection = $activeSection ?? 'mail';
                @endphp
                ...
                <a href="{{ route('admin.settings.section', 'legal-policies') }}"
                        id="tab-legal-policies"
                        class="tab-link px-6 py-3 text-sm font-medium border-b-2 {{ $activeSection === 'legal-policies' ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    <i class="fas fa-balance-scale mr-2"></i> Políticas legales
                </a>
                @endif
                @if($permService->userHasPermission('settings_system', 'view'))
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const currentSection = pathParts[pathParts.length - 1] || 'mail';
    
    // Validar que sea una sección válida
    const validSections = ['mail', 'templates', 'history', 'legal-policies', 'login-lockouts', 'system'];
    const activeSection = validSections.includes(currentSection) ? currentSection : 'mail';
