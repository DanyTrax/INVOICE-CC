    ...
    <div id="modal-assign-quote" class="hidden fixed inset-0 z-50 overflow-y-auto" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
                ...
                <form id="form-assign-quote" method="post" action="" data-action-template="{{ str_replace('999', ':id', route('admin.processes.assign-quote-item', ['process' => 999])) }}">
                    @csrf
                    <div class="mb-4">
                        <label for="assign-quote-item-id" class="block text-sm font-medium text-gray-700 mb-1">Línea de cotización</label>
                        <select name="quote_item_id" id="assign-quote-item-id" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
                            <option value="">Seleccione una cotización / ítem...</option>
                            @foreach($quote_items_for_assign as $qi)
                                <option value="{{ $qi->id }}">
                                    Cotización #{{ $qi->quote->consecutive ?? $qi->quote_id }} | {{ $qi->quote->client->name ?? '-' }} | {{ $qi->serviceType?->name ?? 'Ítem' }}
                                </option>
                            @endforeach
                        </select>
                        @if($quote_items_for_assign->isEmpty())
                            <p class="mt-1 text-xs text-amber-600">No hay líneas de cotización disponibles (todas tienen expediente).</p>
                        @endif
                    </div>
                    <div class="flex justify-end gap-2">
                        ...
                        <button type="submit" class="px-3 py-2 bg-teal-600 text-white rounded-lg text-sm hover:bg-teal-700" @if($quote_items_for_assign->isEmpty()) disabled @endif>
                            Guardar
                        </button>
                    </div>
                </form>
